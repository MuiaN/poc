...
//     other: { label: "O", class: "e-o" },
//   } as const;
//   const cfg = icons[type];
//   return <span className={`e-icon ${cfg.class}`}>{cfg.label}</span>;
// }

// function DetailIcon({ type }: { type: "phone" | "email" | "url" | "location" }) {
//   const icons = {
//     phone: <svg className="ic" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.62 3.38 2 2 0 0 1 3.6 1.18h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.77a16 16 0 0 0 6 6l.95-.95a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>,
//     email: <svg className="ic" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>,
//     url: <svg className="ic" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>,
//     location: <svg className="ic" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>,
//   };
//   return icons[type];
// }
...
function BadgeCustom({ children, bg, color }: { children: React.ReactNode; bg: string; color: string }) {
  return (
    <span style={{ background: bg, color }} className="inline-block text-[10px] font-semibold px-2 py-[1px] rounded-[20px] whitespace-nowrap">
      {children}
    </span>
  );
}
...
function CivilCard({ contact, accentColor, accentBg, accentText }: { 
  contact: ContactCard; 
  accentColor: string; 
  accentBg: string; 
  accentText: string; 
  ...
  return (
    <Card className="p-3.5 border-l-[3px] contact-card" style={{ borderLeftColor: accentColor }}>
      <div className="card-header">
        ...
        <div className="card-title-block flex-1 min-w-0">
          <div className="card-org font-bold text-[12.5px] text-text leading-snug mb-0.5">{contact.org}</div>
          <div className="card-country text-[11px] font-semibold uppercase tracking-wide text-text-2">{contact.country}</div>
        </div>
        <BadgeCustom bg={accentBg} color={accentText}>Civil</BadgeCustom>
      </div>
      <div className="card-divider border-t border-border my-1.5" />
      <div className="card-details flex flex-col gap-1 text-[12px] text-text">
        {contact.details?.map((d, i) => {
          const type = guessDetailType(d);
          return (
            <div key={i} className="card-detail flex items-start gap-1.5">
...
function EmergencyCard({ contact, accentColor, accentBg, accentText }: { 
  contact: ContactCard; 
  accentColor: string; 
  accentBg: string; 
  accentText: string; 
  ...
  return (
    <Card className="p-3.5 border-l-[3px] contact-card" style={{ borderLeftColor: accentColor }}>
      ...
      <div className="card-divider border-t border-border my-1.5" />
      <div className="emerg-rows flex flex-col gap-1">
        {contact.emergRows?.map((row, i) => (
          <div key={i} className="emerg-row grid grid-cols-[18px_1fr] gap-1.5 items-start">
...
function InsuranceCard({ contact, accentColor, accentBg, accentText }: { 
  ...
  return (
    <Card className="p-3.5 border-l-[3px] contact-card" style={{ borderLeftColor: accentColor }}>
      <div className="ins-logo-row flex gap-2.5 mb-1.5">
        <div className="ins-logo w-10 h-10 flex-shrink-0 border border-border rounded-[3px] flex items-center justify-center overflow-hidden bg-bg-3">
          <span className="text-[10px] font-bold text-text-2">{contact.org.slice(0, 2)}</span>
        </div>
        <div className="ins-nb flex-1 min-w-0">
          <div className="ins-name font-bold text-[13px] text-text leading-snug">{contact.org}</div>
          {contact.country && (
            <div className="ins-type text-[11px] font-semibold text-success mt-0.5">Insurance Broker</div>
          )}
        </div>
      </div>
...
function MaintenanceCard({ contact, accentColor, accentBg, accentText }: { 
  ...
  return (
    <Card className="p-3.5 border-l-[3px] contact-card" style={{ borderLeftColor: accentColor }}>
      <div className="card-header">
        <div className="card-flag flex-shrink-0 mt-0.5">
          <img src={`${FLAG_BASE}/${contact.country.toLowerCase().replace(/ /g, '')}.png`} alt="" className="flag-code" width="22" height="15" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
        </div>
        <div className="card-title-block flex-1 min-w-0">
          <div className="card-org font-bold text-[12.5px] text-text leading-snug mb-0.5">{contact.org}</div>
          <div className="card-country text-[11px] font-semibold uppercase tracking-wide text-text-2">{contact.country}</div>
      ...
      <div className="card-divider border-t border-border my-1.5" />
      <div className="card-details flex flex-col gap-1 text-[12px] text-text">
        <div className="card-detail flex items-start gap-1.5">
...
export function Contacts() {
  ...
  const [group, setGroup] = useState<"east" | "horn" | "all">("all");
  ...
  const filteredContacts = useMemo(() => {
    ...
    return result;
  }, [contacts, search, group, showGroupFilter]);
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="tab-bar">
        {TABS.map((t) => (
          <button
            key={t.key}
            onClick={() => { setActiveTab(t.key); setGroup("all"); setSearch(""); }}
      ...
      <div className="cards-grid">
        {filteredContacts.length === 0 ? (
          ...
          filteredContacts.map((c, i) => {
            if (activeTab === "civil") {
              return <CivilCard key={i} contact={c as ContactCard} accentColor={accentColor} accentBg={accentBg} accentText={accentText} />;
            }
            if (activeTab === "emergency") {
              return <EmergencyCard key={i} contact={c as ContactCard} accentColor={accentColor} accentBg={accentBg} accentText={accentText} />;
            }
            if (activeTab === "insurance") {
              return <InsuranceCard key={i} contact={c as InsuranceContact} accentColor={accentColor} accentBg={accentBg} accentText={accentText} />;
            }
            return <MaintenanceCard key={i} contact={c as MaintenanceContact} accentColor={accentColor} accentBg={accentBg} accentText={accentText} />;
          })
        )}
      </div>
    </div>
